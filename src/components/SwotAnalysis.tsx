...
const QUADRANTS = [
  ...
  { key: 'opportunities' as const, label: '기회', sub: 'Opportunities', border: '#2563EB', bg: '#EFF6FF', dot: '#2563EB' },
  ...
  return (
    <div style={{ backgroundColor: '#ffffff', borderRadius: '14px', padding: '28px', boxShadow: '0 1px 6px rgba(0,0,0,0.07)' }}>
      <div style={{ marginBottom: '22px' }}>
        <p style={{ fontWeight: 800, fontSize: '18px', color: '#111827', margin: 0 }}>SWOT 분석</p>
        <p style={{ fontSize: '13px', color: '#9CA3AF', marginTop: '4px' }}>지원자 관점 · 해당 기업 지원 기준</p>
      </div>
          ...
          return (
            <div key={key} style={{ backgroundColor: bg, borderRadius: '12px', padding: '18px 20px', borderLeft: `4px solid ${border}` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px' }}>
                ...
                <span style={{ fontSize: '11px', fontWeight: 500, color: '#9CA3AF' }}>{sub}</span>
              </div>
              {items.length === 0 ? (
                <p style={{ fontSize: '13px', color: '#9CA3AF' }}>데이터 없음</p>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {items.map((item: string, i: number) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                      ...
                      <span style={{ fontSize: '15px', color: '#374151', lineHeight: 1.65 }}>{firstSentence(stripMd(item))}</span>
                    </div>
                  ))}
                </div>
              )}
