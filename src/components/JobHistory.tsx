...
export function JobHistory({ onReconnect }: JobHistoryProps) {
  ...
  // 최초 로드 + 5초마다 자동 새로고침 (리포트 상세 보는 중엔 중단)
  useEffect(() => {
        ...
        <div style={{ backgroundColor: '#ffffff', borderRadius: '14px', padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 1px 6px rgba(0,0,0,0.07)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '28px' }}>
              {[
                { label: '지원 기업', val: selectedJob.company },
                { label: '직무', val: selectedJob.job_title },
                { label: '산업', val: selectedJob.industry },
          ...
          <span style={{ fontSize: '12px', color: '#9CA3AF' }}>{toKST(selectedReport.created_at)}</span>
        </div>
